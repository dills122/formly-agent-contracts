...
Keyless nodes use `::position:` plus their complete zero-based tree position,
including when nested below a keyed group. An inherited model path is not a
semantic key for the group itself. Duplicate keyed IDs receive a deterministic
position suffix and an `UNKNOWN_FIELD_SHAPE` diagnostic rather than being
silently merged.
